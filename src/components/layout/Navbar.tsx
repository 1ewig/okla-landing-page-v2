          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <button
                ...
                className="relative font-body text-[15px] font-medium text-charcoal tracking-[0.2px] hover:text-lime-dim transition-colors duration-[150ms]"
              >
                {link.label}
                {isActive && (
                  <motion.div
                    layoutId="nav-underline"
                    className="absolute -bottom-1 left-0 right-0 h-[2px] bg-lime rounded-full"
                  />
                )}
              </button>
            );
            ...
            <div className="flex flex-col gap-4">
              {NAV_LINKS.map((link) => (
                <button
                  key={link.href}
                  onClick={() => scrollToSection(link.href)}
                  className="font-body text-base font-medium text-charcoal text-left hover:text-lime-dim transition-colors"
                >
                  {link.label}
                </button>
              ))}
